def NED_result_checker():
	...
	find_fy=soup.find('td',text='Computer Science & Info. Tech.').find_next_sibling('td')
	find_sy=find_fy.find_next_sibling('td')
	find_ty=find_sy.find_next_sibling('td')
	find_by=find_ty.find_next_sibling('td')
	#formatted text
	print(FormatText.blue+FormatText.bold+FormatText.underline+"NED Result Checker"+FormatText.end+"\n")
	if find_fy.text=='-':
